...
const RejectedQueuePage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const rejectedData = result.filter((item) => item.status === "Rejected");
          const formattedData = rejectedData.map((item) => ({
            ...item,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status) => (
        <span
          className="px-2 py-1 rounded-[5px] text-xs whitespace-nowrap"
          style={{ backgroundColor: getStatusColor(status), color:"#B91C1C" }}
        >
          {status}
        </span>
      ),
